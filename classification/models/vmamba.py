...
class SS2Dv0:
    ...
    def forwardv0(self, x: torch.Tensor, SelectiveScan = SelectiveScanOflex, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x)
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        # forward_type debug =======================================
        FORWARD_TYPES = dict(
            v01=partial(self.forward_corev2, force_fp32=(not self.disable_force32), SelectiveScan=SelectiveScanOflex), # will be deleted in the future
...
class VSSM(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for layer in self.layers:
            # print('shape before layer', layer, ': ', x.shape)
            x = layer(x)

        # print('x shape before classifier:' , x.shape)
        x = self.classifier(x)
        # print('x after classifier:', x.shape)
        return x
